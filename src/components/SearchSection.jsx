...
function PhoneSearch({ tags }) {
  ...
  }, [displayed, phase, tagIdx, tags]);

  const showResults =
    displayed.length === (tags[tagIdx] ?? "").length ||
    ...
    <>
      ...
      <div className="ss-phone__nav">
        ...
        <div className="ss-phone__nav-tabs">
          <span className="ss-phone__tab ss-phone__tab--active">{t("search.phoneTabAll")}</span>
          <span className="ss-phone__tab">{t("search.phoneTabImages")}</span>
        </div>
        <div className="ss-phone__nav-right">
          <span className="ss-phone__signin">{t("search.phoneSignIn")}</span>
        </div>
      </div>
      ...
      <div className={`ss-phone__results${showResults ? " ss-phone__results--visible" : ""}`}>
        <div className="ss-phone__result">
          ...
          <span className="ss-phone__result-title">{t("search.phoneResult1Title")}</span>
          <span className="ss-phone__result-snippet">{t("search.phoneResult1Snippet")}</span>
        </div>
        <div className="ss-phone__result">
          <span className="ss-phone__result-url">kadyrbekova.md › blog</span>
          <span className="ss-phone__result-title">{t("search.phoneResult2Title")}</span>
          <span className="ss-phone__result-snippet">{t("search.phoneResult2Snippet")}</span>
        </div>
        <div className="ss-phone__result">
          <span className="ss-phone__result-url">kadyrbekova.md › services</span>
          <span className="ss-phone__result-title">{t("search.phoneResult3Title")}</span>
          <span className="ss-phone__result-snippet">{t("search.phoneResult3Snippet")}</span>
        </div>
      </div>
    </>
  );
...
export default function SearchSection() {
  const { t } = useTranslation();
  const tags = t("search.tags", { returnObjects: true });
